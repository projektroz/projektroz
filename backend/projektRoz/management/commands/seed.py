    ...
    def add_parents_notes(self):
        for _ in range(10):
            ...
            Parent.objects.create(
                name=(
                    fake.first_name_male() if role == "M" else fake.first_name_female()
                ),
                surname=(
                    fake.last_name_male() if role == "M" else fake.last_name_female()
                ),
                role=role,
            )
    ...
    def add_children_categories_documents(self):
        ...
        for _ in range(10):
            ...
            Siblings.objects.create(
                child=child, child_sibling=Child.objects.order_by("?").first()
            )
            ...
            Documents.objects.create(
                child=child,
                category=AllowedCategories.choices[
                    randint(0, len(AllowedCategories.choices) - 1)
                ][0],
