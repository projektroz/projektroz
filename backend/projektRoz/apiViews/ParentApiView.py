...
from projektRoz.models import Child, FosterCarer, Parent
...
class ParentApiView(APIView):
    ...
    @swagger_auto_schema(
        manual_parameters=[
            openapi.Parameter(
                "role",
                openapi.IN_QUERY,
                description="Role of the parent ('M' for mother, 'F' for father)",
        ...
        if parent_id:
            ...
            if parent.role == role:
                children = Child.objects.filter(
                    mother=parent, foster_carer=foster_carer
                ) | Child.objects.filter(father=parent, foster_carer=foster_carer)
                if children.exists():
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if role in ["M", "F"]:
                ...
                return Response(
                    {
                        "error": "Invalid role. role must be 'M' for mother or 'F' for father."
                    },
    ...
    def delete(self, request, role, parent_id, *args, **kwargs):
        ...
        if parent.role == role:
            if parent.role == "M":
                children = Child.objects.filter(
                    foster_carer=foster_carer, mother=parent
                )
            else:
                children = Child.objects.filter(
                    foster_carer=foster_carer, father=parent
                )
