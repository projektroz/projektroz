    ...
    @swagger_auto_schema(
        manual_parameters=[
            openapi.Parameter(
                "type",
                openapi.IN_QUERY,
                description="Type of address ('R' for registered, 'U' for unregistered)",
        ...
        responses={200: AddressSerializer(many=True), 404: "Not Found"},
    )
        ...
        if address_id:
            ...
            if address.is_registered == is_registered:
                children = Child.objects.filter(
                    address=address, foster_carer=fosterCarer
                ) | Child.objects.filter(
                    address_registered=address, foster_carer=fosterCarer
                )
                if children.exists():
    ...
    def delete(self, request, type, address_id, *args, **kwargs):
        ...
        if address.is_registered == is_registered:
            if address.is_registered:
                children = Child.objects.filter(
                    foster_carer=foster_carer, address_registered=address
                )
            else:
                children = Child.objects.filter(
                    foster_carer=foster_carer, address=address
                )
